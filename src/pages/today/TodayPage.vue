...
<template>
  <section class="grid gap-5">
    ...
    <div v-if="tasks.myDay.length === 0" class="border border-border bg-surface p-4 text-body text-ink-soft">
      Nenhuma tarefa para hoje.
    </div>

    <template v-for="task in tasks.myDay" :key="task.id">
      <TaskCard v-bind="task" />
    </template>

    <button class="w-fit border border-border bg-surface px-4 py-2 text-body text-ink hover:bg-hover" @click="lockVault">
      Bloquear cofre
    </button>
  </section>
</template>
