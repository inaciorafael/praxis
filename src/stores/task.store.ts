...
export const useTaskStore = defineStore('tasks', {
  ...
  getters: {
    tasksById(state): Record<string, Task> {
      return Object.fromEntries(allKnownTasks(state).map((task) => [task.id, task]))
    },
  ...
  actions: {
    ...
    async applyTaskList(result: TaskListResult, target: TaskListTarget, append = false) {
      this[target] = append ? mergeTasksById(this[target], result.tasks) : result.tasks
    ...
    async hydrate() {
      try {
        ...
        this.error =
          error instanceof Error ? error.message : 'Abra o cofre para carregar tarefas.'
      }
    },
    ...
    async hydrateArchived(options?: TaskListOptions, append = false) {
      try {
        await this.applyTaskList(await listArchivedTasks(options), 'archived', append)
      } catch (error) {
        this.error =
          error instanceof Error
            ? error.message
    ...
    async search(filters: TaskSearchFilters, options?: TaskListOptions, append = false) {
      const requestId = ++this.searchRequestId
      this.isSearching = true

      try {
        const result = await searchTasks(filters, options)
        ...
        this.searchResults = append
          ? mergeTasksById(this.searchResults, result.tasks)
          : result.tasks
        this.searchTotal = result.total
        this.tasks = mergeTasksById(this.tasks, result.tasks)
        this.checklistItems = mergeChecklistItemsForTasks(
          this.checklistItems,
          result.checklistItems,
          result.tasks.map((task) => task.id)
        ...
        this.error =
          error instanceof Error ? error.message : 'Nao foi possivel pesquisar tarefas.'
        return false
      } finally {
        if (requestId === this.searchRequestId) {
          this.isSearching = false
    ...
    async delete(id: string) {
      try {
        ...
        this.error =
          error instanceof Error ? error.message : 'Nao foi possivel remover a tarefa.'
      }
    },
    ...
    async deleteChecklistItem(id: string) {
      try {
        ...
        this.error =
          error instanceof Error ? error.message : 'Nao foi possivel remover o item.'
      }
    },
...
function mergeChecklistItemsForTasks(
  ...
  return [...existing.filter((item) => !refreshedTaskIds.has(item.taskId)), ...incoming]
}
