		...
		async clearNativeLaunchContext() {
			try {
				...
				this.error =
					error instanceof Error
						? error.message
						: "Nao foi possivel limpar contexto da notificacao.";
			}
		},
		...
		async requestPermission() {
			try {
				...
				this.error =
					error instanceof Error
						? error.message
						: "Nao foi possivel solicitar permissao.";
				return false;
			}
		},
		...
		async notifyNow(input: ReminderInput) {
			try {
				...
				this.error =
					error instanceof Error
						? error.message
						: "Nao foi possivel disparar notificacao.";
				return null;
			}
		},
		...
		async schedule(input: ReminderInput) {
			try {
				...
				this.error =
					error instanceof Error
						? error.message
						: "Nao foi possivel agendar notificacao.";
				return null;
			}
		},
		...
		async cancel(id: number) {
			try {
				...
				this.error =
					error instanceof Error
						? error.message
						: "Nao foi possivel cancelar notificacao.";
			}
		},
		...
		async cancelAll() {
			try {
				...
				this.error =
					error instanceof Error
						? error.message
						: "Nao foi possivel cancelar notificacoes.";
			}
		},
	},
});
