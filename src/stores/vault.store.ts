...
import type { DataFileCredentials, DataFileValidation, SafetyCopyInfo, VaultStatus } from "@/shared/types/vault";
...
export const useVaultStore = defineStore("vault", {
  ...
  actions: {
    ...
    async hydrate() {
      try {
        const [status, suggestedPath] = await Promise.all([getVaultStatus(), suggestDataFilePath()]);
        this.applyStatus(status);
        this.suggestedPath = suggestedPath;
      } catch (error) {
        this.error = error instanceof Error ? error.message : "Nao foi possivel carregar o cofre.";
      }
    },
    ...
    async validate(path: string) {
      try {
        ...
        this.error = error instanceof Error ? error.message : "Nao foi possivel validar o arquivo.";
        return null;
      }
    },
    ...
    async selectExistingDataFile() {
      try {
        const selectedPath = await selectDataFilePath(this.selectedDataFilePath ?? this.suggestedPath);
        ...
        this.error = error instanceof Error ? error.message : "Nao foi possivel selecionar o cofre.";
        return null;
      }
    },
    ...
    async selectNewDataFile() {
      try {
        ...
        this.error = error instanceof Error ? error.message : "Nao foi possivel escolher onde criar o cofre.";
        return null;
      }
    },
    ...
    async hydrateSafetyCopies() {
      try {
        const [copies, dir] = await Promise.all([listSafetyCopies(), getSafetyCopiesDir()]);
        this.safetyCopies = copies;
        this.safetyCopiesDir = dir;
        this.error = "";
      } catch (error) {
        this.error = error instanceof Error ? error.message : "Nao foi possivel carregar copias de seguranca.";
      }
    },
    ...
    async reloadFromDisk() {
      try {
        ...
        this.error = error instanceof Error ? error.message : "Nao foi possivel recarregar o arquivo .praxis.";
        return false;
      }
    },
    ...
    async create(credentials: DataFileCredentials) {
      try {
        ...
        this.error = error instanceof Error ? error.message : "Nao foi possivel criar o arquivo .praxis.";
        return false;
      }
    },
    ...
    async open(credentials: DataFileCredentials) {
      try {
        ...
        this.error = error instanceof Error ? error.message : "Nao foi possivel abrir o arquivo .praxis.";
        return false;
      }
    },
    ...
    async close() {
      try {
        ...
        this.error = error instanceof Error ? error.message : "Nao foi possivel bloquear o cofre.";
      }
    },
  },
});
