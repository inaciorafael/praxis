...
export function resolveTagColor(color: string, name: string): TagColorToken {
  const normalizedColor = color.trim().toLocaleLowerCase()
  const preset = praxisTagColors.find(
    (option) => option.text.toLocaleLowerCase() === normalizedColor
  )
  ...
  if (/^#[0-9a-f]{6}$/i.test(normalizedColor)) {
    return {
      key: 'graphite',
      label: 'Personalizada',
      background: `${normalizedColor}26`,
      text: normalizedColor,
    }
  }

  return pickTagColorByName(name)
}
