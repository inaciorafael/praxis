...
export async function syncTaskReminders(
	...
	const pending = reminders
		...
		.map((reminder) =>
			toPendingTaskReminder(reminder, tasksById.get(reminder.taskId)),
		)
	...
	timerReminders.forEach((reminder) =>
		armReminder(reminder, {
			fireIfDue: shouldFireDueReminder(reminder, fireDueReminderIds),
		}),
	);
...
export async function getNotificationLaunchContext(): Promise<NotificationLaunchContext | null> {
	const context = await invoke<NotificationLaunchContext | null>(
		"get_notification_launch_context",
	);
...
export async function initializeNotificationInteractions(
	handlers: NotificationInteractionHandlers = {},
) {
	...
	await listen<NotificationLaunchContext>(
		"praxis://notification-launch",
		async (event) => {
...
function toNotificationOptions(input: ReminderInput, id: ReminderId): Options {
	return {
		...
		sound: input.sound,
		actionTypeId:
			input.payload?.source === "task" ? TASK_REMINDER_ACTION_TYPE : undefined,
		autoCancel: true,
		extra: {
			...input.payload,
			app: "praxis",
...
function compareScheduledAt(current: PendingReminder, next: PendingReminder) {
	return (
		new Date(current.scheduledAt ?? 0).getTime() -
		new Date(next.scheduledAt ?? 0).getTime()
	);
}

function toPendingTaskReminder(
...
function shouldFireDueReminder(
	reminder: PendingReminder,
	...
	return (
		reminder.reminderId !== null && allowedReminderIds.has(reminder.reminderId)
	);
}
