...
export function buildDayStatus(
	...
	const warningWindowMinutes =
		options.warningWindowMinutes ?? DEFAULT_WARNING_WINDOW_MINUTES;
	...
	const reminderTasks = pendingTodayTasks
		...
		.filter(
			(item): item is { task: Task; reminderAt: Date } =>
				item.reminderAt !== null && item.reminderAt.getTime() >= now.getTime(),
		)
		.sort(
	...
	const nextDue =
		dueTasks.find(({ dueAt }) => dueAt.getTime() > now.getTime()) ?? null;
	...
	const base = {
		...
		minutesUntilNextReminder: nextReminder
			? minutesBetween(now, nextReminder.reminderAt)
			: null,
	...
	if (pendingTodayTasks.length > 0) {
		return {
			...
			message: nextReminder
				? `Proximo lembrete em ${formatMinutes(base.minutesUntilNextReminder ?? 0)}`
